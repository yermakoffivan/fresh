...
* **Every settings toggle now says what it changes** - a command palette entry named **Toggle X** changes the editor-wide default and saves it to your config; **Toggle X (Current Buffer)** pins just the active buffer and is remembered per file. Whitespace Indicators, Tab Indicators, Indentation (Spaces ↔ Tabs), Read-Only Mode and Auto-Revert gained the suffix — they always were per-buffer, they just didn't say so.
